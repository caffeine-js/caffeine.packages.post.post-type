...
describe("FindHighlightedPostTypesController", () => {
	...
	it("should find highlighted post types via GET /post-types/highlights", async () => {
		repository.items.push(
			makePostType({ name: "Regular Post", isHighlighted: false }),
		);
		repository.items.push(
			makePostType({ name: "Highlighted Post", isHighlighted: true }),
		);

		const response = await app.handle(
			new Request("http://localhost/post-types/highlights", {
				method: "GET",
			}),
		);
		...
		expect(body[0].name).toBe("Highlighted Post");
	});
});
