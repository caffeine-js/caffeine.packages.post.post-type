...
export class PostTypeRepository implements IPostTypeRepository {
	...
	async findBySlug(slug: string): Promise<IPostType | null> {
		const storedId = await this.cache.get(`${PostType[EntitySource]}::${slug}`);
	...
	async update(postType: IPostType): Promise<void> {
		...
		if (_cachedPostType) {
			...
			await Promise.all([
				this.cache.del(`${PostType[EntitySource]}::$${cachedPostType.id}`),
				this.cache.del(`${PostType[EntitySource]}::${cachedPostType.slug}`),
			]);
		}
