...
export class InMemoryPostTypeRepository implements IPostTypeRepository {
	...
	async findMany(page: number): Promise<IUnmountedPostType[]> {
		...
		return this.items.slice(start, end).map((i) => this.mapToDTO(i));
	}
	...
	private mapToDTO(item: PostType): IUnmountedPostType {
		const { about, data } = unpackEntity<IUnmountedPostType>(item);
		return {
			...about,
			...data,
